...
```{r}
#first shot:
combined_summary <- function(tree, times, l = 1) {
    total_foss = count.fossils(tree[[l]])
    interval_foss =  count.fossils.binned(tree[[l]], times)
    origin_time = tree.max(tree[[l +1]][[l]])
...
tried to change names of result lists... not a good idea
now, try to use the last code, but use '['

```{r}
combined_summary2 <- function(tree, times) {
  
    total_foss = count.fossils(tree, "[")
    interval_foss =  count.fossils.binned(tree, "[", times)
    origin_time = tree.max(tree[[m +1]][[m]])
